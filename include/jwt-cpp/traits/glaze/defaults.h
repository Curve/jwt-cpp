...
namespace jwt {
	 ...
	 */
	inline decoded_jwt<traits::glaze> decode(const std::string& token) {
		return decoded_jwt<traits::glaze>(token);
	}
#endif
	 ...
	 */
	inline jwk<traits::glaze> parse_jwk(const traits::glaze::string_type& token) {
		return jwk<traits::glaze>(token);
	}
